...
const DriverHero = ({ driver, profile, age, team, }: { driver: Driver; profile: Record<string, unknown> | null | undefined; age: number | null; team: Record<string, unknown> | null | undefined }) => (
  <motion.div
  ...
  >
    ...
    <div className="relative flex flex-col md:flex-row gap-8 items-start">
      <div className="w-32 h-32 md:w-40 md:h-40 rounded-2xl bg-gradient-to-br from-primary/30 to-secondary/30 flex items-center justify-center text-6xl md:text-7xl shrink-0">
        {profile?.nationality_code ? getFlagEmoji(profile.nationality_code as string) : driver.countryFlag}
      </div>
      <div className="flex-1">
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            {(profile?.nationality as string) || driver.nationality}
          </div>
          <div className="flex items-center gap-2 text-muted-foreground">
            <Calendar className="w-4 h-4" />
            Born: {profile?.date_of_birth ? new Date(profile.date_of_birth as string).getFullYear() : 'Unknown'} {age ? `(Age ${age})` : ''}
          </div>
          {driver.placeOfBirth && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <MapPin className="w-4 h-4" />
    ...
    <>
      ...
      {profile?.bio && (
        <div className="glass-card p-6">
          ...
          <p className="text-muted-foreground leading-relaxed text-zinc-300 font-sans">{profile.bio as string}</p>
        </div>
      )}

      {/* Career Highlights */}
      {profile?.career_highlights && (profile.career_highlights as string[]).length > 0 && (
        <div className="glass-card p-6">
          <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
          ...
          <ul className="space-y-3 list-disc list-inside text-muted-foreground">
            {(profile.career_highlights as string[]).map((highlight: string, index: number) => (
              <motion.li
                key={index}
      ...
      {(!profile?.career_highlights || (profile.career_highlights as string[]).length === 0) && driver.careerHighlights && driver.careerHighlights.length > 0 && (
        <div className="glass-card p-6">
          <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
            <Medal className="w-5 h-5 text-wec-gold" />
            Career Highlights
    ...
    return (
      <div className="min-h-screen bg-background">
        <SEOHead
          ...
          description={`WEC career profile for ${resolvedDriver.name}.`}
